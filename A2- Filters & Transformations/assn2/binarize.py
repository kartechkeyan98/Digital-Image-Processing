...
def binarize(img, t, vals=[0,1]):
    '''
    **Inputs**
    ----
    - img [np.ndarray | List]: Image matrix of size M x N
    - t [int]: an integer between 0,1,...,K-1 (=255), threshold for binarization
    - vals [list | tuple]: an array containing 2 values one for below threshold and one for above
    ...
    **Description**
    ...
    assert isinstance(vals, (list, tuple)) and len(vals) == 2, "\33[1mvals\33[0m must have 2 values for binarization"

    return np.where(img > t, vals[1], vals[0])
    ...
    **Description**
    ...
    for i in range(0, img_h, stride_h):
        for j in range(0, img_w, stride_w):
            ...
            bin0_block= binarize(block, t_opt, [-1,1])
